...
for r in range(rodadas):
    ...
    # Média dos valores observaveis
    beta_media = np.array([
        [np.mean(y_treino)],
        [0],
        [0]
    ])
    y_pred_media = np.full(y_teste.shape, beta_media[0])  # Mantendo a estrutura de matriz


    # Cálculo do RSS para o modelo da Media
    modelo_media.append(np.sum((y_teste - y_pred_media) ** 2))

    # MQO Tradicional
    # Adicionar coluna de 1s para o intercepto
    X_treino = np.hstack((
        np.ones((X_treino.shape[0],1)),
        X_treino
    ))
    X_teste = np.hstack((
        np.ones((X_teste.shape[0],1)),
        X_teste
    ))
    beta_mqo = np.linalg.pinv(X_treino.T @ X_treino) @ X_treino.T @ y_treino
    y_pred_mqo = X_teste @ beta_mqo

    # Calculo do RSS para o MQO Tradicional
